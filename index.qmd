...
- Hinselmann, Schiller, Citology, Dx, Dx:HPV, Dx:CIN, Dx:Cancer
- STDs: Time since first diagnosis, STDs (number)
- STDs:AIDS, STDs:cervical condylomatosis, STDs:pelvic inflammatory disease, STDs:genital herpes, STDs:molluscum contagiosum, STDs:Hepatitis B, STDs:HPV, STDs:vaginal condylomatosis, STDs:condylomatosis, STDs:vulvo-perineal condylomatosis
...
# Aggregate condylomatosis variants if present
condy_cols = [c for c in ['STDs:condylomatosis','STDs:vulvo-perineal condylomatosis', 'STDs:cervical condylomatosis', 'STDs:vaginal condylomatosis'] if c in data]
if condy_cols:
    data['Any_condylomatosis'] = (data[condy_cols].sum(axis=1) > 0).astype(int).astype('category')
...
#correlation between numberical variable
sns.heatmap(data[num_var].corr(), cmap='vlag')
plt.title('Feature Correlation (Spearman)')
plt.tight_layout()
plt.show()

LeakageVar = ['Hinselmann','Schiller','Citology','Dx','Dx:HPV','Dx:CIN','Dx:Cancer']
highly_correlated = ['STDs: Time since first diagnosis','STDs (number)']
VeryRare = ['STDs:AIDS','STDs:cervical condylomatosis','STDs:pelvic inflammatory disease',
              'STDs:genital herpes','STDs:molluscum contagiosum','STDs:Hepatitis B',
              'STDs:HPV','STDs:vaginal condylomatosis', 'STDs:condylomatosis','STDs:vulvo-perineal condylomatosis'] #<1%

#dropping the column
data = data.drop(columns=LeakageVar)    
data = data.drop(columns=highly_correlated)   
data = data.drop(columns=VeryRare)             
```
...
#update cat_var 
cat_var = data.select_dtypes(include=['object', 'category'])
#update num_car
num_var = data.select_dtypes(exclude=['object', 'category'])
...
# TRAIN/VAL split for RandomizedSearchCV 
...
for name, base_model in models:
    ...
    random_search = RandomizedSearchCV(
        ...
        n_iter=n_iter_config.get(name, 30),  
        cv=cv_tune, 
        scoring=SCORING,
        n_jobs=-1,
        refit=True,
    ...
    print(f"\nTraining {name} with {n_iter_config.get(name, 30)} iterations...")
    random_search.fit(X_train_val, y_train_val)

    best_by_model[name] = (random_search.best_estimator_, random_search.best_params_, float(random_search.best_score_))
    print(f"{name}: best VAL {SCORING}={random_search.best_score_:.4f}")
    print(f"Best params: {random_search.best_params_}")
    
    val_score.append((name, float(random_search.best_score_), random_search.best_params_))
...
# 1. Get the preprocessor from the best pipeline
preprocessor = best_estimator.named_steps['preprocessor']

# 2. Get the model's final classifier
classifier = best_estimator.named_steps['classifier']

# 3. Preprocess the data for SHAP
# Ensure you are using the full dataset for explanation
...
explainer = shap.LinearExplainer(classifier, X_train_val_processed)
...
For practical application, the model's utility could be enhanced by focusing on improving its ability to detect the rare positive cases. Future work should explore strategies such as:
  1. decision-threshold calibration guided by precision–recall trade-offs
  2. Incorporating richer clinical variables (e.g., specific HPV strain information) to improve the model's predictive power.
