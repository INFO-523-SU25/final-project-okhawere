...
sequenceDiagram
    ...
    Note over Data, Processor: Data Cleaning Pipeline
    
    Data->>Processor: Raw Dataset
    Processor->>Processor: Drop 23 duplicate participants
    Processor->>Processor: Remove leakage & high correlation columns (18 Columns)
    Processor->>Processor: Eliminate rare/missing feature columns 
    
    Note right of Processor: Handling Systematic Missingness
    Processor->>Processor: Impute missing values<br/>based on common variables
    
    Note right of Processor: Feature Engineering
    Processor->>Processor: Create "Years since first intercourse"
    
    Processor-->>Data: ✅Cleaned & Enhanced Dataset
